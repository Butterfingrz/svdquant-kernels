...
![生产 shape 上的 Tensor Pipe % —— 我们 fp16 与 nunchaku fp16 的 elapsed 落在 0.2 pp 之内](figures/preface_table_zh.png)

数字来自 `ncu` 的
`sm__pipe_tensor_cycles_active.avg.pct_of_peak_sustained_*` 计数器 ——
tensor 管线在**每张卡自己的单周期上限**上有多忙，跟 boost 时钟无关。
我们本来用 cross-arch MFU，但只好放弃：nunchaku 只编 SM_120a/121a
（消费/工作站 Blackwell，见 `nunchaku/setup.py:41-64`），没有 SM_100
二进制，所以这个对比天然是数据中心 Blackwell vs 消费 Blackwell ——
消费卡的 boost 时钟摆幅足以让 MFU 数读漂移大过代码质量的差
（`docs/gpu.md § Cross-arch MFU caveat`）。

读法：ours-fp16 与 nunchaku-fp16 把各自的 tensor pipe 饱和到 0.2 pp
之内（45.2 vs 45.4）—— **两边都把自己手上的硬件吃满到差不多的程度**。
5.8× 用时差是 B200 更多 SM + 更高的单周期 FP4 峰值在起作用，**不是
代码质量差**。nunchaku 的 bf16 elapsed 高出 fp16 elapsed 9.2 pp，
是因为他们的 fp16 hand-PTX 碰到了 register cliff（255 寄存器、
2.28 M LMEM spill，详见 §9.2）；我们两条 dtype 都没碰到这个 cliff。

同台 B200、剥掉 LoRA 和仿射、CUTLASS 的
`dense_blockscaled_gemm_persistent.py` 在 2-CTA 256×256 上跑到
45 %–63 % MFU —— 这是**同卡** MFU，不受跨芯片 clock-drift 影响，
是真正还值得追的天花板。
...
Server Edition（SM_120a）上跑它作为**实现品质参考**，不是上限。

§1 那张表就是 apples-to-apples 的读数：`ncu` 的
`sm__pipe_tensor_cycles_active.avg.pct_of_peak_sustained_*`
在生产 shape `M=4352 K=3840 N=3072 R=128` 上。每一行三个数字，三件事：

- **elapsed % 一样**（45.2 vs 45.4）。墙钟意义上，两条内核都把各自的
  tensor pipe 饱和到 0.2 pp 之内。这是 kernel-quality 读数 —— 两边
  都把手上的硬件吃到自己单周期上限的同样比例。
- **active % 不一样**（fp16 上 52.0 vs 58.8）。SM 有活干的那些周期里，
  nunchaku 的 hand-PTX 每个 active cycle 塞进去的 tensor 活更多；
  我们有更多的 bubble cycle。这是 `docs/gpu.md:79-103` 说的结构性
  DSL-vs-PTX codegen 差：nunchaku 的 MMA 是 inline PTX
  （`mma_earlycuda.cuh`），fp16 和 bf16 是两套分别手调的路径，register
  packing / 累加器精度不同；我们走一个 `tcgen05` atom + `ab_dtype`
  替换，两种 dtype 走同一份 MLIR。要补 active % 这一段大概率得下到
  inline PTX，超出范围。
- **5.8× 用时差**是硬件能力：B200 SM_100 比 PRO 6000 SM_120a 多 SM、
  单周期 FP4 峰值更高。这不是 kernel-quality 读数。

**为什么这里不再贴 cross-arch MFU**。MFU（FLOPS / 设备峰值）需要一个
稳定的设备峰值。消费 Blackwell（PRO 6000、RTX 50 系）的持续时钟随热
包络和单 die binning 摆得很厉害 —— 规格表里"峰值 FLOPS"对应的是一个
特定时钟，运行时实测时钟会上下漂。这一节先前有过一张 4-shape 的
cross-arch MFU 表，我们重跑了一遍发现 clock-drift 的信号已经盖过
kernel-quality 的信号，就撤了。详细写法在
`docs/gpu.md § Cross-arch MFU caveat`。

**关于 nunchaku 的 fp16 列**。**他们这一列内部**，bf16 elapsed
（54.6 %）比 fp16 elapsed（45.4 %）高 9.2 pp，是 register-pressure
cliff：他们的 fp16 hand-PTX 打到 255 寄存器/线程 + ~2.28 M LMEM
spill + 101 % spill overhead；bf16 路径 248 寄存器、零 spill。那
7 个寄存器的差就是"装得下/装不下"的差。我们两个 dtype 都没碰到这种
cliff —— 单条 `tcgen05` atom + `ab_dtype` 替换走同一份 MLIR，两个
dtype 同源 codegen，所以我们 fp16 elapsed ≈ 我们 bf16 elapsed。
这是他们参考实现在消费 Blackwell 上的 codegen 性质，不是我们内核的
性质。

报告：`log/ncu_v2_postLUfix_4352_3840_3072_R128.ncu-rep`（B200），
